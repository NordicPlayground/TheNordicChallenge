...
export const Strava = () => {
	...
	return (
		<Main>
			<>
			<div style={{width: "400", height:"200", display: 'flex', justifyContent: 'space-around', alignItems: 'center'}}>
				<div>
				<h1>Distance goal</h1>
				...
				<table className="HoursTable">
					...
					<tbody>
					{(weeklyHoursSorted.slice(0,4)).map(
							(item: {
								name: string
								hours: number
							}) => (
								<tr>
									<td style={{padding: '2px 10px'}} key={item.name}>{item.name.split('-').pop()}</td>
									<td style={{padding: '2px 10px'}} key={item.hours}>{Math.round(item.hours)}</td>
								</tr>
							),
						)}
					</tbody>
